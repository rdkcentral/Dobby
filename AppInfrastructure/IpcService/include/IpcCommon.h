...
/**
 * @brief Helper class to send reply of a method call asynchronously.
 */
class IAsyncReplySender
{
public:
    virtual ~IAsyncReplySender() = default;

    /**
     * @brief Get method call arguments.
     */
    ...
    /**
     * @brief Get Id of the user (unix user Id) who invoked the method.
     */
    virtual uid_t getSenderUid() const = 0;

    /**
     * @brief Gets the unique name of the connection which originated this message.
     */
    virtual std::string getSenderName() const = 0;
};

/**
 * @brief Helper class to get reply.
 */
